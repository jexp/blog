...
As this is released in different part on our blog, it would be great to have in the intro :

A reminder about what this blog post series is about
What we covered in the part 1 (with a link)

| Then you need the motherduck extension and you’re ready to go. I just put that into my $HOME/duckdbrc config file. 
This sound a bit confusing as one could understand they need to do something extra on the setup. 
Maybe rephrase and do something in these lines :  anywhere you can run duckdb you can run motherduck as the motherduck extension is part of the official duckdb extensions and will be downloaded & loaded as soon as you try to connect to a motherduck database through .open md:`
There's a problem with an image, getting a 404 here
...
As the MotherDuck extension

Anywhere you can run DuckDB you can use MotherDuck as its extension is an official DuckDB extension and will be downloaded & loaded as soon as you connect to a motherduck database through `.open md:` or similar commands.
...
Once you have an account you get a *access token*, which you need to connect to MotherDuck.
...
So we can rather create the database on the MotherDuck UI and import our tables directly from our Parquet files on S3.
