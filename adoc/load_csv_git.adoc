...
We create the commit, connect it to its author and the day it happend on which we merge teh path to the time-tree root (year) on the fly.
As last step we split out the parent commit list into a collection and link them to the commit.
